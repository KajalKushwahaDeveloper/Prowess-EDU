...
import { studentLogin } from "../../features/auth/studentAuthReducer.js";
...
const StudentLoginScreen = () => {
  ...
  // Select Redux state
  const { data, loading, error } = useSelector((state) => state.studentAuth);

  useEffect(() => {
    if (data?.status === 200) {
      localStorage.setItem("token", data?.data?.token);
      localStorage.setItem("data", JSON.stringify(data?.data?.student));
